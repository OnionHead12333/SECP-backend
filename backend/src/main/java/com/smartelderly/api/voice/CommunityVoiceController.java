...
@RestController
@RequestMapping("/v1/community-voice")
public class CommunityVoiceController {
    ...
    public CommunityVoiceController(InterestCommunityService interestCommunityService,
    ...
    public ResponseEntity<byte[]> file(@PathVariable String messageId, HttpServletRequest request) throws IOException {
        var user = SecurityUtils.requireRole(UserRole.elder);
        // 兼容：历史上私聊误走社群语音 URL（/v1/community-voice/direct_xxx/file）
        if (messageId != null && messageId.startsWith("direct_")) {
            var payload = elderSocialService.loadVoiceMedia(user, messageId);
        ...
        var info = interestCommunityService.resolveMediaFileInfo(elder.getId(), messageId, "voice");

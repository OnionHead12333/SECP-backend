...
public class ElderUserProfileController {
    ...
    @PostMapping("/avatar")
    public ApiResponse<ElderUserProfileView> uploadAvatar(@RequestParam("file") MultipartFile file) {
        var p = SecurityUtils.requireRole(UserRole.elder);
        return ApiResponse.ok("uploaded", elderUserProfileService.uploadAvatar(p.userId(), file));
    }
}

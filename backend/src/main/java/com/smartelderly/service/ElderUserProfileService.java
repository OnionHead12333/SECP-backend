...
import com.smartelderly.service.profile.ProfileAvatarStorageService;
...
public class ElderUserProfileService {
    ...
    @Transactional
    public ElderUserProfileView uploadAvatar(long elderUserId, MultipartFile file) {
        User user = userRepository.findById(elderUserId)
                .orElseThrow(() -> new ApiException(4010, "unauthorized"));
        if (!"elder".equalsIgnoreCase(user.getRole())) {
            throw new ApiException(4030, "forbidden");
        ...
        if (file == null || file.isEmpty()) {
            throw new ApiException(4001, "请上传头像文件");
        }
        String avatarUrl;
        try {
            avatarUrl = profileAvatarStorageService.save(user, file);
        } catch (IOException e) {
            throw new ApiException(5001, "保存头像失败：" + e.getMessage());
        }
